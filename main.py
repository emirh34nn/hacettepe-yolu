...
# GÜN SONU TEMİZLİĞİ (Sadece bir kez çalışır)
if 'init_clean_361' not in st.session_state:
    st.session_state.gunluk = []
    save_json([], FILES["gunluk"])
    st.session_state.init_clean_361 = True

st.set_page_config(page_title="HACETTEPE YOLU v3.6.1", layout="wide")
...
if choice == "📊 Dashboard":
    ...
    st.info(f"🚀 TYT 2026 Hedefine: **{fark.days} Gün** Kaldı!")
...
# --- 2. YKS NET ANALİZÖRÜ (v3.5.9'un aynısı) ---
...
# --- 3. EDEBİYAT OYUNU (v3.5.9'un aynısı) ---
...
# --- 4. GÜN SONU KRİTİĞİ (Özel İstediğin Sade & Akıllı Versiyon) ---
    ...
    st.info(f"📅 Bugün: **{bugun.day} {aylar_tr[bugun.month]} {bugun.year}**")
    col1, col2 = st.columns(2)
    with col1:
        with st.form("gunluk_form", clear_on_submit=True):
            secilen_tarih = st.date_input("Kritik Tarihi", value=bugun, max_value=bugun)
            saat = st.number_input("Kaç Saat Çalıştın?", 0.0, 24.0, 5.0)
            verim = st.slider("Verim (1-10)", 1, 10, 7)
            notlar = st.text_area("Günün Özeti")
            if st.form_submit_button("Günü Mühürle"):
                yeni = {"tarih": str(secilen_tarih), "saat": saat, "verim": verim, "not": notlar}
                st.session_state.gunluk = [k for k in st.session_state.gunluk if k['tarih'] != str(secilen_tarih)]
                st.session_state.gunluk.append(yeni)
                save_json(st.session_state.gunluk, FILES["gunluk"]); st.success("Mühürlendi!"); st.rerun()
    with col2:
        if st.session_state.gunluk:
            st.dataframe(pd.DataFrame(st.session_state.gunluk).sort_values("tarih", ascending=False), use_container_width=True)

# --- 5. SORU EKLE (v3.5.9'un aynısı) ---
...
# --- 6. SORU ARŞİVİ (v3.5.9'un aynısı - ARAMALI) ---
...
# --- 9. KİTAP TAKİBİ (v3.5.9'un aynısı - EKLEMELİ/SİLMELİ) ---
