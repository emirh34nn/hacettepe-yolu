...
st.set_page_config(page_title="HACETTEPE YOLU v3.2.1", layout="wide")
...
# --- 1. DASHBOARD ---
if choice == "📊 Dashboard":
    st.title("📈 Başarı & Hatırlatıcı")
    bugun_str = datetime.now().strftime("%d/%m/%Y")
    tekrar_listesi = [s for s in st.session_state.sorular if s.get('tekrar_tarihi') == bugun_str]
    
    if tekrar_listesi:
        st.warning(f"🔔 Bugün tekrar etmen gereken {len(tekrar_listesi)} kritik soru var!")
    
    c1, c2, c3 = st.columns(3)
    c1.metric("Soru Sayısı", len(st.session_state.sorular))
    c2.metric("Kart Sayısı", len(st.session_state.kartlar))
...
# --- 2. SORU EKLE ---
    ...
    with st.form("yukle_form", clear_on_submit=True):
        ...
        with col1:
            ...
            ders = st.selectbox("Ders", ders_list)
            yayin = st.text_input("Yayın")
        with col2:
            sayfa = st.number_input("Sayfa", 1)
            soru_no = st.number_input("Soru No", 1)
        ...
        if submit and resim_f:
            ...
            yeni = {"id": random.randint(1000,9999), "ders": ders, "yayin": yayin, "sayfa": sayfa, "soru_no": soru_no, "resim": encoded_img, "cevap": dogru_sik, "hac_puani": hac_puani, "not": notum, "tekrar_tarihi": tekrar_tarihi}
            st.session_state.sorular.append(yeni)
            save_json(st.session_state.sorular, DB_FILE)
            st.success("Soru Başarıyla Mühürlendi!")
            st.rerun()

# --- 3. SORU ARŞİVİ ---
elif choice == "🔍 Soru Arşivi":
    st.header("🔍 Soru Bankan")
    search = st.text_input("Ders veya Yayın Ara...")
...
# --- 4. DENEME KAYDI ---
    ...
    with st.form("deneme_f"):
        ...
        if st.form_submit_button("Netleri İşle"):
            ...
            save_json(st.session_state.denemeler, DENEME_FILE)
            st.success(f"Toplam Net: {total}")
            st.rerun()

# --- 5. SÖZEL PRATİK KARTLAR ---
elif choice == "🗂️ Sözel Pratik Kartlar":
    st.header("🗂️ Sözel Hafıza Kartları")
    with st.form("k_form", clear_on_submit=True):
        k_ders = st.selectbox("Ders", ["Edebiyat", "Tarih", "Coğrafya", "Felsefe"])
        k_on = st.text_input("Soru / Kavram")
        k_arka = st.text_area("Cevap")
        if st.form_submit_button("Karta Mühürle"):
    ...
    for k in reversed(st.session_state.kartlar):
        ...
        if st.button("🔄 Çevir", key=f"f_{k['id']}"):
            st.info(f"**Cevap:** {k['arka']}")
        st.divider()

# --- 6. HAC GÜNÜ 2.0 ---
elif choice == "🔥 HAC Günü 2.0":
    st.header("🎲 Haftalık Kamp")
    if len(st.session_state.sorular) < 5: st.warning("5 soru biriktir!")
    else:
        if st.button("Rastgele 5 Soru Getir"):
            st.session_state.hac_secimi = random.sample(st.session_state.sorular, 5)
